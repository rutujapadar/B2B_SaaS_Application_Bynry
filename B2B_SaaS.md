...
The corrected code ensures input validation, checks business constraints (SKU uniqueness), and enforces **atomicity** using a single database transaction.
...
def create_product():
    ...
    # 1. Input Validation and Safe Access
    ...
    # 2. Business Logic Validation (SKU Uniqueness)
    # ASSUMPTION: 'Product.query.filter_by(sku=sku).first()' is the ORM method
    if Product.query.filter_by(sku=sku).first():
        return {"error": f"Product with SKU '{sku}' already exists."}, 409 # 409 Conflict

    # 3. Transaction Block for Atomicity
    try:
        # Create new product (WITHOUT warehouse_id, as it's a property of Inventory)
        product = Product(
            name=data['name'],
            sku=sku,
            price=data['price'],
            # Note: Assuming 'company_id' is derived from the user session/context.
        )
        ...
        # 4. Single Commit: Both Product and initial Inventory creation must succeed or fail together.
        db.session.commit()
        
        return {"message": "Product created successfully", "product_id": product.id}, 201

    except Exception as e:
        # 5. Rollback on Failure: Ensures no half-created records are left behind.
        db.session.rollback()
        # In a real app, log 'e' here.
...
CREATE TABLE Warehouses (
    ...
    UNIQUE (company_id, name) -- Ensures unique warehouse names per company
);
...
CREATE TABLE Inventory_History (
    ...
    quantity_change INT NOT NULL,          -- Magnitude of the change (+/-)
    new_quantity INT NOT NULL,             -- Resulting stock level
...
### 💡 Justification of Decisions
  ...
  * **`Inventory` Table Constraint:** The use of `UNIQUE (product_id, warehouse_id)` is the fundamental design choice that satisfies the multi-warehouse requirement while ensuring data integrity (a product can only have one stock count in a given warehouse).
  ...
  * **`Inventory_History`:** This table is essential for the "Track when inventory levels change" requirement. It serves as an immutable ledger, recording the **transaction\_type** and **quantity\_change** for auditing and calculating historical velocity (used in Part 3).
  * **`Product_Suppliers` (`is_primary_supplier`):** The M:N table allows for sourcing flexibility, and the boolean flag is an indexable field that significantly simplifies the logic needed to quickly find the correct supplier for the low-stock alert.
...
const db = {
    query: async (sql, params) => { 
        // ... Actual database connection/query logic ... 
...
app.get('/api/companies/:company_id/alerts/low-stock', async (req, res) => {
    ...
    try {
        ...
        // Efficiently joins Inventory, Products, Warehouses, and Primary Supplier data
        const lowStockQuery = `
            SELECT
